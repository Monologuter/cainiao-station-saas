...
> 版本：v1.1 ｜ 日期：2026-06-17 ｜ 状态：6 项开放决策已拍板回填，待评审
...
### 5.1 identity（认证与权限）
- **职责**：登录、JWT 签发/刷新、账号、RBAC（角色-权限-菜单）；收件人(Consumer)身份认证（小程序微信登录 / H5 手机号验证码）独立于员工 RBAC 体系。
...
| 期 | 主题 | 范围 | 交付物（可上线/可演示） |
|---|---|---|---|
| **P1** | MVP 核心闭环 | tenant(手动开店)+identity(RBAC)+station(货架库位)+inbound+parcel(状态机)+pickup+notify(站内/模拟短信)+user-app(查件取件)+基础统计 | 一家驿站能真用：入库→通知→取件全流程跑通 |
