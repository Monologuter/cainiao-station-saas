...
> 版本：v1.3 ｜ 日期：2026-06-18 ｜ 状态：6 项决策 + 权限体系 + 三主题切换体系已回填，待评审
...
### 8.4 多主题体系（系统内可切换）
平台内置 **3 套可切换主题**，本质是 **设计 token（CSS 变量）集**，三套共用**同一套组件库与布局**，切换只换 token、瞬间生效、按用户记忆偏好——不是维护三个前端。
- **清爽蓝**（`blue`，浅色 · 飞书蓝 `#3B6EF6`，中等圆角）
- **科技暗**（`dark`，深色 · 青色霓虹 `#22D3EE`，发光点缀、网格微光底）
- **柔和薄荷**（`mint`，浅色 · 薄荷 `#19B894`，大圆角、柔和阴影）
- **Token 维度**：色板(primary/bg/surface/text/border/状态色) + 圆角 + 阴影 + 字体(`--font`/`--font-head`/`--font-num`) + 辉光(`--glow`)。
- **实现**：根节点 `data-theme` 切换；偏好存 `localStorage` + 后端用户配置（跨设备）；默认 `blue`。三套主题在每个组件上都要验收（尤其暗色对比度）。
- **范围**：station-web / admin-web 全量支持；user-app（uni-app）至少支持 浅/暗 两态，主题 token 同源复用。

### 8.5 前端公共
- 共享 `request`（Axios 封装、租户头、错误码处理）、`api types`（与后端 contracts 对齐）、权限指令/路由守卫、**主题 token 包**（三端复用同一份设计变量）。
- **布局规范：全屏平铺（full-bleed）**——后台采用「固定侧栏 + 内容区自适应铺满剩余全宽」，内容贴边、占满整个可视区，**不做居中固定宽的内嵌容器**；宽屏自适应。所有页面遵循此规范。
...
| 7 | UI 主题 | **3 套可切换主题**（清爽蓝 / 科技暗 / 柔和薄荷） | token 化设计变量，一套组件库三套皮肤，系统内切换 + 记忆偏好，详见 8.4 |
