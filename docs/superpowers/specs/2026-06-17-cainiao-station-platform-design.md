...
| 用户端 | `user-h5` | 收件人/寄件人 | 手机查件、出示取件码、在线寄件、物流追踪 |

### 1.3 运营模式
- **多租户**：一个「租户」= 一个驿站经营主体（老板/品牌），下可挂 1~N 个门店。
- **入驻**：P1 由平台手动开店；P3 开放自助申请→审核→开通。
- **变现**：按门店月度订阅（基础/标准/旗舰套餐）+ 增值（短信条数、寄件抽佣、AI 能力等），P3 落地。
...
| 前端 | Vue3 + TS + Vite + Pinia + Vue Router | 3 个独立应用 |
| 前端 UI | Element Plus（两后台）/ Vant（H5） | ECharts 图表 |
...
### 5.1 identity（认证与权限）
- **职责**：登录、JWT 签发/刷新、账号、RBAC（角色-权限-菜单）。
- **实体**：`User`（含 type: platform/staff）、`Role`、`Permission`、`UserRole`、`RolePermission`。
- **接口**：`login / refresh / logout`、`@RequirePermission()` 守卫、当前用户/租户上下文注入。
- **权限模型**：平台角色（平台管理员/运营/客服）与租户角色（店长/店员，可自定义）分离；权限码形如 `parcel:inbound`、`station:shelf:manage`。
- **依赖**：tenant（解析归属租户）。
...
### 5.3 billing（订阅计费）— P3
- **职责**：套餐、订阅、账单、用量计量（短信条数、包裹量）、欠费停用。
...
- **渠道**：站内消息（DB）/ 模拟短信（控制台+记录，今天可用）/ 真实短信（阿里云，资质后）/ 微信模板（认证后）/ H5 推送（Socket.IO）。
...
- `roles`(id, tenant_id?, code, name, scope[platform|tenant])
...
### 8.3 user-h5（用户端）
- **定位**：收件人/寄件人手机使用，Vant 移动端体验。
- **关键页**：手机号登录（验证码）/ 我的包裹（待取/已取/历史）/ 取件码出示 / 代取授权 / 在线寄件 / 物流追踪 / 消息中心 / 会员积分。
- **扫码**：`vue-qrcode-reader` 调摄像头。
...
- **鉴权与权限**：JWT（access + refresh）；RBAC 数据模型见第 6 节；守卫 `@RequirePermission`；平台/租户角色隔离。
...
| 期 | 主题 | 范围 | 交付物（可上线/可演示） |
|---|---|---|---|
| **P1** | MVP 核心闭环 | tenant(手动开店)+identity(RBAC)+station(货架库位)+inbound+parcel(状态机)+pickup+notify(站内/模拟短信)+user-h5(查件取件)+基础统计 | 一家驿站能真用：入库→通知→取件全流程跑通 |
...
1. **租户多门店**：一个租户允许多门店（小连锁），还是 P1 先限定「一租户一门店」？（建议数据模型支持多门店，P1 UI 先按单门店做）
2. **用户端跨店查件**：确认收件人按手机号可跨门店聚合查件（更真实，但需专用跨租户只读通道）？
3. **小程序**：用户端 P1 用 H5，未来是否要做微信小程序（影响是否提前留小程序登录态口子）？
4. **代收货款 COD**：是否需要（涉及资金与对账，建议 P3+）？
5. **订阅计费模式**：按门店月费 / 按包裹量阶梯 / 混合？（P3 细化）
6. **短信服务商**：阿里云 / 腾讯云 / 其他（影响 P4 适配实现，接口已抽象）？
