...
> 版本 v1.1（2026-06-18）｜ 设计级别（不含完整实现代码）
> 配套《设计方案（整合版）v2.0》§2/§9/§12、《实现计划总表》P3-4
> 技术栈锁定：NestJS + TypeScript｜Redis 7（令牌桶限流 / 多级缓存 / 分布式锁）｜可观测（结构化日志 + Prometheus 指标 + 健康检查）
>
> **实现状态校正（v1.1）**：本文档为设计目标态。落地实现与设计存在差异，凡涉及可观测三方库处均已就地标注 `【实现：…】`。汇总：
> - 日志：**未引入 pino**，使用 Nest 内置 `Logger` + `AsyncLocalStorage`（`core/tenant-context`）贯穿 `traceId/tenantId/userId`（见 `core/observability/trace.interceptor.ts`）。
> - 指标：**未引入 prom-client**，`core/observability/metrics.service.ts` 自实现 Prometheus 文本暴露格式（`/metrics` via `metrics.controller.ts`）。
> - 健康检查：**未引入 @nestjs/terminus**，`core/observability/health.controller.ts` 自实现 `/health/live` + `/health/ready`（PG `SELECT 1` + Redis `PING`）。
> - 错误监控：**Sentry 未落地，且当前代码无任何 Sentry 占位/DSN 配置**，仅有全局异常过滤器 + 错误指标；Sentry 为后续待落地项。
...
3. **可观测性**：结构化日志 + 请求链路 ID（traceId）贯穿、`/health` 健康检查、`/metrics` Prometheus 指标、错误监控。【实现：日志用 Nest 内置 `Logger`（非 pino）+ ALS 贯穿 traceId；`/metrics` 自实现暴露格式（非 prom-client）；`/health` 自实现（非 terminus）。】
...
> **【实现校正】目标态为 pino；当前未引入 pino**，使用 Nest 内置 `Logger`。`traceId/tenantId/userId` 已通过 `AsyncLocalStorage`（`core/tenant-context`）+ `core/observability/trace.interceptor.ts` 贯穿并回写 `X-Request-Id` 响应头。下文 pino 相关（JSON 序列化、`pino-pretty`、redact 序列化器）为**目标态/待落地**，迁移到 pino 时落地。

- **统一 logger（目标态）**：基于 pino 封装 Nest `LoggerService`，全应用 JSON 结构化输出（生产 JSON、本地 `pino-pretty`）。
...
> **【实现校正】未引入 `@nestjs/terminus`**，由 `core/observability/health.controller.ts` 自实现，两端点已落地：`/health/live` 直接返回、`/health/ready` 检查 PostgreSQL（`SELECT 1`）+ Redis（`PING`）。MinIO / BullMQ / AI-service 就绪检查为**目标态/待落地**。

分两端点：
- `GET /health/live`（存活）：进程在即 200，供容器存活探针。【已实现】
- `GET /health/ready`（就绪）：检查依赖——PostgreSQL（`SELECT 1`）、Redis（`PING`）【均已实现】；MinIO（可选）、BullMQ 队列连通、AI-service（可选、降级不阻断就绪）【目标态/待落地】。任一关键依赖不通返回 503，供 LB/就绪探针摘流。
- 返回结构含各依赖状态明细，纳入运维看板。

### 5.4 Prometheus 指标（`/metrics`）

> **【实现校正】未引入 `prom-client`**，`core/observability/metrics.service.ts` 自实现 Prometheus 文本暴露格式（`Content-Type: text/plain; version=0.0.4`），`core/observability/metrics.controller.ts` 暴露 `/metrics`，`metrics.interceptor.ts` 记录 HTTP 请求计数与时延。**当前已落地指标**：`http_requests_total{method,route,status}`、HTTP 时延汇总、`scheduled_job_skipped_total`。下表其余指标与 `collectDefaultMetrics`（进程内存/GC/句柄）为**目标态/待落地**（自实现注册表逐步补齐或后续切 prom-client）。

目标态指标清单，暴露 `/metrics`（仅内网/带 token）：
| 指标 | 类型 | 标签 |
|---|---|---|
...
> **【实现校正】Sentry 未落地，且当前代码中无任何 Sentry 占位 / `SENTRY_DSN` 配置项**（既非已接入、也尚未预留 DSN）。当前仅有全局异常过滤器 + 错误指标；Sentry（含 DSN 配置化）为**后续待落地项**。

- 全局异常过滤器统一捕获 → 结构化 error 日志（含 traceId）+ `errors_total{type}` 指标。【过滤器已实现；error 日志走 Nest `Logger`；错误指标随 §5.4 自实现注册表补齐】
- Sentry 对接（DSN 配置化）：**待落地**，本期仅日志+指标。
...
| 敏感脱敏 | 日志/响应统一脱敏：手机号（138****5678）、取件码、身份证、token；响应拦截器脱敏白名单【目标态：pino 序列化器 redact 路径，待迁 pino 后落地，当前用 Nest `Logger`】 |
...
| # | Task | 关键产物 | 验收 |
|---|---|---|---|
| 1 | **可观测性地基**（先做，后续 Task 复用日志/指标）【已落地，详见 §5 各节实现校正】 | `core/observability`：logger（**实现为 Nest `Logger`，非 pino**）、traceId 拦截器 + ALS 注入、全局异常过滤器接错误指标、`/health(live/ready)`、`/metrics`（**自实现暴露格式，非 prom-client**）、HTTP 指标拦截器 | 日志含 traceId/tenantId；`/health/ready` 反映 PG/Redis 状态；`/metrics` 可被 prom 抓取；单测 + 拦截器集成测绿 |
...
| 脱敏遗漏 | 敏感信息泄漏日志 | 响应脱敏白名单 + code-review + 测试覆盖关键字段【目标态：统一 pino redact，待迁 pino 后补】 |
