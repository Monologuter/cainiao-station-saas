...
### 8.1 执行落地记录（2026-06-18）

- [x] Task 1：ai-service `/slot/recommend` 与评分排序完成，pytest 通过（提交 `7ac62f1`）。
- [x] Task 2：后端 `slot-allocator` 接入智能推荐与规则回退，单测/e2e/build 通过（提交 `a4675c4`）。
- [x] Task 3：`slot_heat_daily` + RLS + 取件事件热度聚合 + `/stations/:id/slots/heatmap` 完成（提交 `944f58b`）。
- [x] Task 4：ai-service `/forecast/volume` 与后端 `volume_forecasts` + RLS + 查询/重算接口、`ForecastProcessor` 定时预测防重与昨日 `actualVolume` 回填完成（提交 `010165b`、`f4a1441`，收口提交见本轮）。
- [x] Task 5：station-web 入库推荐提示、货架热力、经营统计预测看板，以及 `smart-slot-forecast.e2e-spec.ts` 集成冒烟完成（提交 `a8ec390`）。

**验收结果**：`ai-service` 预测/库位 pytest 已通过；backend `slot-allocator`/`slot-heat`/`inbound`/`forecast.processor` unit、`smart-slot-forecast`/`forecast`/`station-loop` e2e、`npx prisma validate`、`npm run build` 通过；station-web 50 个 Vitest、`npm run build` 通过。MAPE 看板展示当前以 `actualVolume` 回填数据为基础，后续可继续在 analytics API 增加滚动误差聚合字段。
