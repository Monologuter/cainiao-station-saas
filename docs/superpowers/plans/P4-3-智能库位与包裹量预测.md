...
> 上游规格：`docs/superpowers/specs/cainiao-station-platform-design.md`（§5 station / inbound / ocr·ai / analytics · §6 数据模型 · §7 适配层 · §9 横切 · §10 入库流程）；路线图：`docs/superpowers/plans/全周期实现计划总表.md`（P4-3）；复用历史数据基建：`P2-4-运营大屏.md`（`metric_daily` / 货架热力 / 滞留榜）；接入点上游：`P1-2-驿站核心闭环.md`（`slot-allocator.service`）。
